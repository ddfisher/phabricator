create database if not exists {$NAMESPACE}_notification;

create table if not exists {$NAMESPACE}_notification.notification_storydata(
...
drop table if exists {$NAMESPACE}_notification.notification_subscribed;

create table {$NAMESPACE}_notification.notification_subscribed(
  id int unsigned not null auto_increment primary key,
...
insert into {$NAMESPACE}_notification.notification_subscribed
...
insert ignore into {$NAMESPACE}_notification.notification_subscribed
  (userPHID, objectPHID, lastViewed)
